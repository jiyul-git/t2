...
## 7. 정정 (`TRACE_EQREL_COUNT.md`)

1,000핸드 집계 결과, 3절의 "`442`·`458` 에서 `rel` 로 탈락" 은 **다수
사례에서 맞지 않는다.** `465` 로 온 36건 중 25건은 `rel >= 0.52` 라
`458` 의 `rel` 문턱을 넘고, 그 건들을 막은 것은 `made >= 1` 이다.

`made` 도 현재 보드만 보는 값이므로 구조 진단(진입 = 미래 포함 `eq`,
내부 = 현재 보드만)은 유지된다. 다만 **내부 심사의 구속 조건은 `rel` 이
아니라 `made` 로 좁혀야 한다.** 본문은 고치지 않고 여기 기록을 남긴다.
