...
# 수트를 기호로. 'Qs' 보다 'Q♠' 가 한눈에 들어온다.
_SUIT = {'s': '\u2660', 'h': '\u2665', 'd': '\u2666', 'c': '\u2663'}


def K(card):
    c = str(card)
    return c[0] + _SUIT.get(c[1:2].lower(), c[1:2]) if len(c) >= 2 else c


def KS(cards):
    return '  '.join(K(c) for c in (cards or []))
...
def render_result(res, hero=None, hand_no=None, notes=None, bb=None):
    ...
    if res.get('showdown'):
        for s, hl in res.get('hole', {}).items():
            ...
            L.append('  쇼다운  %s번  %s%s' % (s, KS(hl), mark))
    else:
