...
## STEP 6 — my_range 역할 매핑 수정 (known issue C 해결)

C 자체는 버그가 아니었다. **C 를 발생시킨 upstream 역할 매핑이 버그였다.**

    pf_role 정보는 정상적으로 존재 (h.pf_seed[s]['pf_role'])
      → session.py 가 그것을 사용하지 않음
      → 포스트플랍 aggressor 로 프리플랍 role 을 재추정
      → BB 가 플랍에서 베팅 → role='open'
      → BB 는 프리플랍에 오픈한 적이 없다
      → BB/open 호출 70회가 **전부 빈 레인지**
      → nut_adv/range_adv 결손, opp_r 폴백까지 오염

수정: `pf_role` 을 사용(open/iso→open, defend→call). 단 **BB 는 예외** —
BB 의 iso 는 프리플랍 오픈이 아니라 이미 블라인드로 들어와 있는 상태에서
올린 것이라 레인지 기반이 call 쪽이다. `_base_open('BB')` 가 사실상 0 이라
`pf_range` 개념이 8.8 이어도 빈 레인지가 됐다.
내 레인지(363행)와 **상대 레인지(372행)** 양쪽 모두 같은 버그였다.

### 성공 조건 8개

| # | 조건 | 결과 |
|---|---|---|
| 1 | 매핑 정확 | pf_role 사용 + BB 예외 |
| 2 | 불필요한 변화 없음 | 46건 중 45건 설명, D 1건 |
| 3 | BB open 제거 | 70 → 0 |
| 4 | my_range=[] 제거 | 7/708 → 0/713 |
| 5 | 지표 정상 계산 | range_adv 255/678 변화 |
| 6 | 변화 검토 가능 | A 35 / B 10 / D 1 |
| 7 | 재현성 유지 | 6항목 OK |
| 8 | STEP 5-A 회귀 | PASS, unavailable 1→0, skipped 4→0 |
...
D 1건(시드41005 #22 turn BTN)은 my/opp/rel/adv 가 **전부 동일한데**
fold → check 로 바뀌었다. fold 는 저항이 있을 때만, check 는 없을 때만
나오므로 **그 시점에 도달한 경로가 달라진 것**으로 보인다(앞선 액션 변화의
결과). 판단이 이상해진 것이 아닐 가능성이 크나 확정하지 않았다.


## STEP 7 — 같은 스트리트 안의 레인지 갱신 (known issue B 해결)

`refreshed` 가드가 '스트리트당 한 번'이라 **상대 레인지가 같은 스트리트
안에서 좁혀져도 지표를 갱신하지 않았다.** 보드 의존 지표에는 맞는 가드지만
레인지 의존 지표에는 맞지 않았다.

수정: 갱신 조건에 레인지 변화(`_rsig`)를 추가. 같으면 재계산하지 않는다.
`_rsig` 는 `update_plan`/`revise_plan` 승계 목록에도 넣었다.

**내 레인지는 서명에서 뺐다.** 그 스트리트 안에서 내용이 변하지 않는데
매번 새 리스트로 만들어져 서명만 흔들린다(3건이 그 때문에 변화로 오판).
실제로 좁혀지는 것은 상대 레인지다.

### 계측기부터 고쳤다

첫 계측은 `update_plan` 호출 순서대로 이웃끼리 비교해 **서로 다른 좌석의
레인지를 비교**했다(A/turn → B/turn → A/turn 에서 A 와 B 를 비교).
좌석 x 스트리트 단위로 바꾸자 판정이 뒤집혔다 —
'진짜 stale 17건' 이 실은 `레인지 동일 → 미갱신(정당)` 17건이었다.

| | 전 | 후 |
|---|---|---|
| 레인지 변화 → **미갱신(stale)** | **8** | **0** |
| 레인지 변화 → 갱신(정당) | 27 | 33 |
| 레인지 동일 → 미갱신(정당) | 17 | 17 |

### 호출 증가 21%의 정체

계산 호출이 743 → 902 로 늘어 '낭비 47건' 으로 보였으나, 호출마다 원인을
스택으로 태깅하니 **refresh 경로의 불필요한 재계산은 0회**였다.

    new_street          71.4%   새 스트리트 첫 계획
    make_plan(revise)   15.4%   board_changed 로 계획 재생성
    first_plan           4.6%   그 스트리트 최초 생성
    opp_range_changed    2.6%   이번 수정의 목적
    other(n2)            6.1%   decide_aggression 자체 계산
    unknown(refresh)     0%

47건은 전부 `make_plan(revise)` 였다. 계획이 새로 만들어지므로 재계산이 맞다.
'낭비' 라는 라벨이 부정확했다.


## known issue E — pot_control 승격 조건의 made 자기참조 (수정하지 않음)
...
`st.update` 가 위에서 `st['made']` 를 새 값으로 덮어쓴 뒤라 이 식은
`made >= made + 1` 이 되어 **모든 made 값에서 거짓**이다. 실질 조건은
`rel >= 0.70` 단독이고, 주석에 적힌 '완성 강도가 올라갔는가' 는 구현되지
않은 상태다. 실측(6시드 270핸드): 분기 대상 109건 중 made 항 기여 0건.

### _prev_made 로 고쳐봤으나 되돌렸다

수정하자 신규 승격 11건이 발생했는데 검토 결과 대부분 승격시키면 안 되는
자리였다.

```
Ad8h9s9h    보드 9 페어   made 1→2  rel 0.52  eq 0.397
5c8cKc4c8d  보드 8 페어   made 1→2  rel 0.44  eq 0.177
2d5h9h9s    보드 9 페어   made 1→2  rel 0.00  eq 0.031  ← 승률 3% 인데 밸류 승격
```

`made 1→2` 가 **내가 핸드를 개선한 경우와 보드가 페어링된 경우를 구분하지
못한다.** 보드 페어 + 내 페어로 숫자만 오른 것이라 실질 강도는 그대로다.

### 왜 threshold 로 막지 않는가

`made 상승 AND eq >= X` 로 막으면 그 X 가 결국 실전 결과에 맞춰 만든 새
보정값이 된다. 지금까지 피해온 방식이다.

### 진짜 문제

단순 stale-variable 버그가 아니라 **made 를 승격 신호로 사용하는 설계 자체의
한계**다. 필요한 것은
- Hero 의 실제 hand class 가 개선됐는가
- 그 개선이 상대 레인지에 대해 의미가 있는가
를 분리해 정의하는 것이다. threshold 하나 추가로 될 문제가 아니다.

### #27 수정과의 차이

`44a0ebc`(value_2street → value_3street)는 유지한다. #27 은
`made 3→7 / rel 1.00 / eq 1.00`(실제 풀하우스)로 **현재 상태가 이전 계획보다
압도적으로 강해졌다는 명확한 증거**가 있었고, 조건에 `rel >= max(0.85, 이전 rel)`
과 예산 소진을 함께 요구한다. E 는 `made 1→2 / rel 0.00 / eq 0.031` 까지
같은 신호로 취급하는 것이 문제였다.

**교훈: made 의 숫자 변화 자체를 '강도 상승' 으로 취급하면 안 된다.**


## known issue D — nut_adv 음수 (조사 종결: 버그 아님)

#27 리버 `Jc As Kd 2d Ks` 에서 Hero KK(풀하우스, rel 1.00, eq 1.00)인데
`nut_adv = -0.58`. 계측으로 재현한 결과 **정상 계산**이었다.

```
내 레인지 165개   상위구간 점유 0.927 / 0.309   (UTG+1 오픈, 3c3d 같은 약한 포켓 포함)
상대 레인지  83개  상위구간 점유 1.000 / 0.434   (플랍·턴 콜로 압축, 전부 상위 구간)
nut = (0.5*(0.927-1.0) + 0.5*(0.309-0.434)) * 6 = -0.592
```

`nut_advantage` 는 **range vs range** 의 강한 구간 점유율 차이지
Hero 의 현재 핸드 강도가 아니다. 상대가 두 번 콜해 약한 조합이 빠지면
Hero 가 실제로 풀하우스를 들고 있어도 레인지 우위는 음수일 수 있다.

`bluff_mode` 의 양극화 조건과 `decide_size` 가 이 값을 쓰는 것도 현재
정의에서 일관된다. **nut_adv 를 Hero 의 현재 핸드 강도와 섞어 해석하지 말 것.**
