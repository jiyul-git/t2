...
LOADING = {
 ...
 # 트랩은 개념 자체(숨긴다)는 본능에 가까워 진입 장벽이 낮다. 대신 경험으로
 # 붙는다. **숨겨서 실제로 밸류를 뽑아내는 실행**은 별개이고 그쪽이 공부를
 # 요구한다 — 그 층은 checkraise_flop/late 가 맡는다(trap_judgment 가 둘을
 # 함께 쓴다). 예전에는 study 0.25 / exp 0.40 이라 공부로도 경험으로도 잘
 # 늘지 않아 사실상 노이즈로만 결정됐다.
 'trap':              (0.20,-0.10, 0.65, 4.4),
