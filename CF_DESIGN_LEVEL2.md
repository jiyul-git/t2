...
### 5-2. D arm 의 계획층 내부 실행층 함수 호출

D intervention 에서도 M 과 반대 방향의 구조적 결합이 존재한다.

`make_plan` 내부에서 다음 실행층 함수가 호출된다.

```
barrel_size     <- make_plan       plan.py:480
bluff_mode      <- make_plan       plan.py:497
opp_bet_prob    <- trap_judgment   plan.py:224   (trap_judgment <- make_plan:408)
```

따라서 D intervention 을 "실행층에서만 작동하며 계획층과 독립적" 이라고
기술하는 것은 코드 구조를 정확히 표현하지 않는다.

다만 이것은 **D 가 계획 라벨을 변경한다는 의미가 아니다.** 본측정 및
독립 8시드·2시드 검증에서 D arm 의 `plan≠` 는 7축 전부 정확히 0 이었다.

따라서 현재 관측으로 확정할 수 있는 것은 다음 두 명제를 구분하는 것이다.

```
1  구조적 의존성   D 경로가 계획 구축 과정에서 실행층 함수의 산출물을
                  참조한다.                              → 확인됨
2  계획 라벨 변경   D intervention 이 최종 plan 을 변경한다. → 관측되지 않음
```

즉 D 는 계획층과 코드 수준에서 완전히 격리되어 있지는 않지만, 현재
측정에서 D intervention 에 의한 `plan` label mutation 은 확인되지 않았다.

**"D 가 계획층과 완전히 독립적이다" 라고 쓰지 않는다.** 정확한 진술은
"D 의 계획층 내부 호출이라는 구조적 결합은 존재하지만, 그것이 최종 계획
라벨 변경으로 나타난 증거는 없다" 이다.

`plan≠ = 0` 의 근거:

```
55시드 본측정         7축 모두 0.0%
8시드 (5000-5007)     7축 모두 정확히 0
5000-5001 jobs=1/2    두 실행의 전 칸 동일
```

5-1 과 대칭이다.

```
M   계획층 산출물이 실행층 행동으로 전달될 수 있음  → plan=,act≠ 관측
D   실행층 함수가 계획 구축 과정에서 호출됨         → 그러나 plan≠ 는 관측되지 않음
```

이 항목을 기록하는 이유는 ③ 의 숫자를 뒤집기 위해서가 아니라 설계 문서의
의미를 정확하게 만들기 위해서다.
