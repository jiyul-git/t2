...
**잠재와 어긋난 조합이 만들어진다.** 반사실 스왑은 `latent`(study·aggro·exp)를
그대로 두고 축만 1 또는 9 로 바꾼다. `aggression` 은 `r:aggro = 0.938`,
`R² = 0.880` 이라 `aggro 3 인데 aggression 9` 같은 **필드에 실재하지 않는
조합**이 된다. `discipline`(R² 0.106)·`looseness`(0.141)는 잠재와 거의
독립이라 그 문제가 작다.
