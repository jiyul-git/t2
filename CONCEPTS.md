...
### 블로커 — 핵심은 '강한 콤보'가 아니라 '콜할 콤보'

```
블로커   상대의 콜 몫을 지운다   -> 블러프가 잘 통한다
언블로커 상대의 폴드 몫을 지운다 -> 남은 레인지가 강해져 손해다
```

같은 카드가 상황에 따라 둘 다 된다.

K♠9♠4♠2♦7♥ 리버에서 J♠T 로 블러프하면, J♠ 가 상대의 **약한 플러시**를 지운다.
그건 큰 벳에 접었을 패다. 접을 패를 지웠으니 상대의 남은 레인지가 강해지고
블러프가 덜 먹힌다. 강한 콤보만 세는 방식으로는 이 손해가 보이지 않는다.

`ranges.blocker_effect(hero, opp_range, board, street, size_frac, for_value)`
가 `_call_range` 로 콜/폴드를 가르고 각 몫에서 지운 비율의 차를 낸다.
**밸류는 부호가 뒤집힌다** — 콜해줘야 벌 수 있으므로 콜 몫을 지우면 손해다.

K♠9♠4♠2♦7♥ 리버 팟사이즈 벳:

| 내 패 | 옛 점수 | 블러프 | 밸류 |
|---|---|---|---|
| K로 탑페어 (콜 지움) | 0.17 | **+0.077** | −0.077 |
| J♠ 약플러시 | 0.07 | +0.051 | −0.051 |
| A♠ 넛블로커 | 0.08 | +0.031 | −0.031 |
| 2로 최약 (접을패 지움) | 0.06 | **−0.081** | +0.081 |

`blocker_score`(강한 콤보만 셈)는 근사로 남긴다 — 상대 벳 사이즈를 모를 때의 폴백.

---

### 리버 — 전용 계획 둘

`SIZING` 표에서 `bluff_2street` 와 `value_2street` 의 리버가 **0.0** 이었다.
이름 그대로 2스트리트 계획이라 리버를 안 치는 것이 맞지만, 그 결과
**리버에서 새로 시작하는 라인이 통째로 없었다.**

| 계획 | 언제 |
|---|---|
| `river_bluff` | 미스한 드로우. busted 드로우는 리버 블러프의 대표 후보다 |
| `thin_river` | 얇은 밸류. rel 0.48~0.92 대역에서만 |

**순서 문제도 있었다.** `refresh` 가 `outs < 6` 으로 semibluff 를 강등하는데
리버는 outs 가 항상 0 이라 무조건 걸렸다. `river_fix` 가 돌기 전에
이미 giveup 이 되어 블러프 전환 경로가 막혀 있었다.
리버에서는 강등을 보류하고 `river_fix` 에 넘긴다.

리버 블러프 전환율 (bluff/barrel_river 개념별): 12% / 22% / 52%.
얇은 밸류 전환율 (rel 0.75, thin_value_river 개념별): 13% / 29% / 54%.
...
## 인지 모델 taxonomy와 설계 원칙 (2026-09-05 진단 결과)

### 원칙: study 는 행동을 직접 결정하지 않는다

```
study → 이 사람이 어떤 개념을 얼마나 이해하는가 (concept competence)
      → 그 개념이 상황을 만나 판단으로 쓰임
      → 행동으로 창발
```

`study → VPIP` 같은 직접 경로가 강하면 **오히려 잘못된 모델이다.**
진단에서 `study` 와 행동 지표의 상관이 -0.11~+0.12 로 사실상 0 이었는데,
이건 버그가 아니라 설계대로다. 반면 `aggro` 는 `temper.aggression` 으로
거의 1:1 전달되므로 행동에 나타난다(+0.18~+0.21). 성격은 직접, 인지는 간접.

**앞으로 코드를 고칠 때도 이 기준을 지킨다.** 관찰 모델이 잘 안 맞는다고
해서 `study` 를 행동에 직접 연결하면 모델이 망가진다.

### 왜 지금은 개념이 행동으로 잘 안 드러나는가

개념이 행동이 되려면 **그 개념을 쓸 상황이 시뮬레이션 안에 있어야 한다.**

| 개념 | 필요한 상황 | 현재 |
|---|---|---|
| `board_texture` | 플랍 텍스처·드로우·메이드·레인지 우위 | 부분 |
| `icm` | 버블·페이아웃·스택 분포·쇼브 압박 | 부분 |
| `potodds` | 실제 가격 대 에퀴티 비교가 반복되는 콜/폴드 | 부분 |
| `range_read` | 상대 라인 누적과 레인지 좁히기 | 부분 |

지금은 프리플랍 비중이 커서 대부분의 개념이 표현될 자리가 부족하다.
**모든 개념을 지금 행동에 연결하려 하면 안 된다.** taxonomy 만 정해두고,
현재 구현된 상황에서 관측 가능한 것부터 작동시킨다.

### taxonomy (구현 여부와 무관하게 자리만 잡아둠)
...
내부 구조를 더 파는 대신 **실전 플레이 → 리뷰 → 문제를 개념 단위로 분류
→ 그 개념만 보강** 순서로 간다. 실제로 문제가 난 개념만 모델에 추가한다.
