...
## Amendment A1 — fixture 타당성 결함 두 개 (전체 실행 전)

작성 시점: fixture 구현 직후 진단에서 결함을 발견한 시점. **전체(n=400) 실행 전.**

**공개**: 결함을 찾기 전에 소규모 예비 실행(n=30)의 상관값을 이미 봤다.
숨기지 않는다. 아래 수정은 그 수치를 좋게 만들려는 것이 아니라 fixture 가
사전등록 3절이 요구한 것("최적 행동이 `b_true` 에 따라 뒤집히도록 잡는다")을
실제로 만족하지 못했기 때문이다. 판정 문턱·규칙·시드는 바꾸지 않는다.

### A1-1 regret 이 읽기가 아니라 **행동 빈도**를 재고 있었다

진단 (프로필 40개, READ 96 상태):

```
regret 와 행동빈도(콜률/벳률)의 상관   r = −0.858
regret 와 looseness                     r = −0.473
regret 와 aggression                    r = −0.245
READ 상태의 최적 행동 분포              fold 13 / call 35 · check 14 / bet 34
```

최적 행동이 공격 쪽에 2.6배 몰려 있어 **그냥 더 콜하고 더 치는 프로필이
읽기와 무관하게 낮은 regret 를 받는다.** 이 상태로는 "QX 가 EV 와 연결되는가"를
물을 수 없다 — 종속변수가 읽기 품질이 아니라 성향이다.

**수정**: READ 의 (채널 · 히어로 · 보드 · 표본) 조합마다 **가격 `sz` 를 탐색해
최적 행동이 `b_true` 격자 안에서 뒤집히도록** 잡는다. 목표는 4개 `b_true`
수준 중 2개가 공격 최적, 2개가 수비 최적이 되는 `sz` 다. 그러면 주변 분포가
균형을 이루고 **`b_true` 를 읽는 것 말고는 이길 방법이 없다.**

이것은 사전등록 3절 READ 정의의 구현이지 정의 변경이 아니다.

### A1-2 A 채널이 `b_true` 에 거의 반응하지 않았다

같은 진단에서 봇 행동의 `b_true` 에 따른 변동폭이
**D 0.492 / A 0.026** 이다. A 채널은 `plan='giveup'` 으로만 불러서
`cbet_freq` 이탈 경로를 타고 `fold_equity` 블록에 도달하지 못했다 —
`NUMERIC_BUNDLE_V7_RESULT.md` 1-1 에서 고친 것과 **같은 결함을 다시 냈다.**

**수정**: `QX_E` 의 A 계열과 같은 방식으로 `giveup` 과 `semibluff`(outs=9)
두 경로를 다 부르고 평균한다. QX 측정과 EV 측정이 같은 코드 경로를 봐야
비교가 성립한다.

### A1-3 정상 작동이 확인된 것

`need` 가 `b_true` 를 따라가는 상관이 **−0.657** 이다 (블러프가 많은 상대일수록
콜 문턱이 낮다). production 읽기 파이프라인 자체는 의도대로 돈다.
문제는 fixture 의 목표 분포였지 읽기 경로가 아니다.
