...
## Amendment A1 — 반사실 설계 확정 (반사실 실행 전)

작성 시점: 분포 calibration 종료 직후, **반사실 코드/실행 전.**
적합값 `c5 = 0.7087277777777778` 은 이미 동결됐고 여기서 바꾸지 않는다.

### A1-1 5단계 커버리지 문제와 선택

`field.field_quality` 가 entries 에 의존한다. 사전등록한 반사실 조건
(entries 100)에서 시드 `880001..880012` 12필드 전체의 5단계 인원이 **1명**이다
(880009 에 1명). 그대로 돌리면 5단계 경로가 사실상 측정되지 않는다.

사용자 결정: **B — 사전등록 반사실은 그대로 두고 400인 보조 반사실을 추가한다.**

```
주 반사실 (사전등록, V6 와 직접 비교 가능)   entries 100 · 시드 880001..880012
보조 반사실 (5단계 커버리지 전용)            entries 400 · 시드 890001..890012
```

`890001..890012` 는 기존 어느 집합과도 겹치지 않는다. 보조 쪽 수치는
**보조로만 읽고 주 반사실의 결론을 대체하지 않는다.**

### A1-2 5절 표의 모호한 곳 두 군데를 구현 전에 고정한다

표의 "trait | prior 보조"(4단계)와 "정밀 채널 | detail 이 준 것만"(4단계)이
겹치는 지점이 있다. 아래로 확정한다. 수치를 바꾸는 결정이 아니라 표현 확정이다.

```
PRIOR(tier, k)
  2   coarse6[k]  단 k ∈ {open_gap, passive},  그 외 0
  3   coarse6[k]  단 k ∈ COARSE6,              그 외 0
  4   (1−a_k)·coarse6[k] + a_k·trait[k]   단 k ∈ COARSE6,  그 외 0
  5   (1−a_k)·coarse6[k] + a_k·trait[k]   단 k ∈ COARSE6,
      아니면 trait[k] (trait 이 정의한 것),     그 외 0

EVIDENCE(tier, k)
  2,3  없음 (detail 을 아예 안 본다)
  4    qual5(detail[k])      채널의 V6 clamp 범위 위 5점 격자
  5    detail[k]             연속 원값

out[k] = (1 − data_eff)·s·PRIOR + data_eff·EVIDENCE
         data_eff = data (tier 4·5) / 0 (tier 1~3),  s = E
```

즉 **trait 은 4단계에서 coarse6 채널의 prior 를 보조할 뿐이고, 정밀 채널에
trait 이 들어가는 것은 5단계뿐이다.** 4단계의 정밀 채널은 detail 에서만 온다.

`a_k` 는 채널이 속한 축의 `see_*` 다 (freq/line/size). V6 와 같은 묶음을 쓴다.

`s = E` 는 **gap 성 채널에만** 곱한다. `size_river` 는 상대 사이즈의 절대값이라
중립 기본 0.62 를 유지하고 `s` 를 곱하지 않는다.

### A1-3 1단계의 `w`

"상대별 조정 거의 0" 을 모호하지 않게 하려고 **1단계는 `w = 0.0` 으로 낸다.**
채널이 전부 0 인 데다 소비처 가중까지 0 이므로 상대별 조정이 구조적으로 없다.
`see_*` 는 진단용으로 그대로 싣는다.
