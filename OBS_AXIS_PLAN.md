...
> **이 축을 가진 플레이어가, 그 축이 실제로 관여하는 동일한 판단
> 상황에서, 반복적으로 예측 가능한 행동 방향성을 보이는가?**

"플레이어별 행동 분산이 큰가"가 **아니다.** 제각각이기만 한 것은
좋은 결과가 아니다. 축 값을 알면 그 사람의 관측 행동을 얼마나
좁힐 수 있는가가 질문이다.

**"그 축이 실제로 관여하는 상황에서"** 가 핵심이다. 축마다 코드상
담당 구간이 다르므로, 담당 밖 상황을 분모에 넣으면 축을 억지로 전체
행동의 설명자로 만들게 된다. 3.2 에서 `cbet_flop` 의 분모를 한 번
잘못 잡았다.
...
(`persona.py`). **`thin_value_turn` 은 턴 전용이 아니라 플랍+턴 전체**다.

```
plan.py:941    p *= (0.55 + 0.09 · sk)
  sk=0  → 0.55
  sk=9  → 1.36      비율 2.47×     ← 실효 범위
  sk=10 → 1.45      비율 2.64×     ← 정의역 상한
```

축은 0~10 으로 clamp 되지만 필드 분포가 평균 4.50 / sd 1.87 이라 9 이상이
드물다. **실효 배율은 2.47× 로 읽는다.** 셋 중 표본이 가장 넉넉하다.
...
### 해석 주의 — `cbet_flop` 의 ρ 가 작을 때

틀린 해석:

> `cbet_flop` 축이 실제 플랍 공격성에 영향을 주지 않는다

맞는 질문:

> `cbet_flop` 축이 **자신이 담당하는 이탈 지속벳 상황에서** 행동
> 방향성을 만드는가

이 세 축은 전체 벳 행동을 설명하는 축이 **아니다.** 관측 지표는
"계획상 포기/쇼다운이지만 이니셔티브가 있어서 계속 공격하는 상황에서의
행동 확률" 이다. 분모가 좁아진 만큼 실현 행동만으로 ρ 를 내면
표본 부족과 실행 랜덤성으로 쉽게 오판한다 — `f` 층을 반드시 같이 본다.

### 이름과 적용 범위가 어긋나는 축

> **`thin_value_turn`** — 플랍 및 턴 밸류 베팅의 thin-value 성향을
> 조절하는 축이다. 이름은 내부 concept 명칭을 따른 것이며 **적용 범위는
> 턴에 한정되지 않는다** (`persona.street_concept` 이
> `('thin_value','flop')` 을 이 축으로 보낸다).

"턴 전용 축"으로 재면 플랍 신호를 통째로 버린다.

배율을 적을 때는 **원식과 실효/정의역 두 값을 같이** 적는다. 정의역
상한(sk=10)만 쓰면 필드에 거의 없는 구간의 숫자가 되고, 실효값만 쓰면
식이 사라진다.
...
| `semibluff` | `semibluff` 계획 비율 | 드로우 보유 포스트플랍 계획 | ⌐ | **`entries` 선택으로 살릴 수 없는 축.** 게이트 1.33 아래가 e=24 에서도 2.1%, e=250 에서 0.6% 다. 대비집단이 없어서 축 버킷 비교 자체가 성립하지 않는다 — 별도 취급(게이트 아래 표본을 따로 모으거나 `f` 층만 본다). 드로우 강도(`outs`) 층화도 필요하다: 확률식이 `0.25 + 0.24×개념` 이라 8아웃과 13아웃이 같이 굴려진다 |
...
```
단변량 Spearman ρ     전체 관측 연관성
잠재3 통제 편상관       잠재요인으로 설명되지 않는 추가 연관성
축 고유 분산 1 − R²    생성모델 내부에서 이 축이 갖는 독립 정보의 몫
```

셋은 서로 다른 것을 잰다. **고유 분산이 작다는 것은 행동 전달 경로가
약하다는 뜻이 아니라 측정축의 독립성이 낮다는 뜻이다.**
...
**전달 경로 자체는 굵다.** `aggression` 은 `decide_aggression` 의
밸류 분기(`0.30 + 0.058·aggression`)와 `pot_control` 분기
(`0.18 + 0.035·aggression`)에 직접 계수로 들어간다. 경로 관점에서
명확히 살아 있는 축이다 — 고유 분산이 작은 것과 별개 문제다.
...
`◎` 잴 수 있다 · `△` 기회 수에 따라 reliability 확인이 필요하다 ·
`✕` 이번 라운드에서는 그 층을 분석하지 않는다

| 축 | 판단 `f` | 실현 행동 | 근거 |
|---|:---:|:---:|---|
| `aggression` | ◎ | ◎ | 밸류·`pot_control` 분기 전체 |
| `looseness` | 조건부 | ◎ | 프리플랍. `defend_thresholds` **tot** |
| `thin_value_turn` | ◎ | △ | 플랍+턴 밸류, `rel<0.85` |
| `bluff` | ◎ | △ | 블러프 계획 분기 |
| `cbet_flop` | ◎ | △ | 이탈 지속벳만 |
| `barrel_turn` | ◎ | △ | 같음, 턴 |
| `barrel_river` | ◎ | **✕** | 인당 기회 중앙 0.0 — 5개 조건 전부 |
| `potcontrol` | ◎ | 조건부 | 스위치. 게이트 아래 집단 크기에 달렸다 |
| `blockbet` | ◎ | △ | `block` 계획 |
| `reraise` | ◎ | △ | 포스트플랍 벳 직면 |
| `discipline` | ◎ | △ | 이탈 지속벳 감쇠 |

`✕` 는 **축이 죽었다는 뜻이 아니다.** observable 의 해상도가 부족한 것이다.

#### 분모는 평균이 아니라 **분포**로 싣는다

`cbet_flop` 계열은 분모가 좁아져서 평균이 의미를 잃는다. 인당 평균
기회가 5회여도 분포가

```
0회 30%  ·  1~2회 25%  ·  3~5회 25%  ·  6~10회 15%  ·  10+ 5%
```

이면 절반 이상이 ρ 에 기여하지 못한다. **축마다 인당 분모의 히스토그램과
split-half 신뢰도 `r` 을 같이 낸다.** 평균만 싣지 않는다.
