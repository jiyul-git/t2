...
## 부록 A. Amendment A1 — 파이프라인 pilot 이후 (calibration 시드 실행 전)

작성 시점: **calibration 시드(820001..820024) 를 한 개도 돌리기 전.**
버림 시드 999124 / 999125 두 개로 덤프→피팅→평가 전 구간을 한 번 돌려
도구가 도는지만 확인했다. 두 시드는 calibration/holdout 어디에도 넣지 않는다.

pilot 에서 드러난 것 하나를 보고 항목으로 추가한다. **적합 규칙·상수·게이트는
바꾸지 않는다.** 3-4 의 이름 배정 규칙도 그대로다. 추가되는 것은 점검 한 줄뿐이다.

pilot 에서 3-4 의 1번(`MANIAC := Xz 최대`)이 **극단 공격 군집이 아닌 군집**을
집어가는 경우가 관측됐다. 표본에 진짜 극단 영역이 없으면 X 가 상대적으로만 높은
평범한 군집이 MANIAC 이름을 받고, 그 결과 holdout 의 MANIAC 빈도가 올라간다.
이것은 MANIAC 탐지가 살아난 것이 아니라 **라벨 이동**이다. 사용자가 경계한
"MANIAC 을 만들어내는" 결과와 겉모습이 같아진다.

**C6 (보고 전용, 재배정 금지)**

```
C6  MANIAC 군집 중심의 L 과 A 가 둘 다 모집단 평균 이상이다
```

C6 가 깨지면 배정을 고치지 않는다. 대신 결과 문서에 다음을 명시한다.

> V2 의 MANIAC 중심은 극단 공격 영역이 아니다. 따라서 V2 의 MANIAC 빈도는
> MANIAC 탐지력의 증거가 아니라 이름 배정의 결과다.

C6 는 어떤 적합값도 바꾸지 않는다. 출력에 불리언 한 개가 늘 뿐이다.

또 하나, pilot 에서 확인한 구조적 사실을 미리 적어 둔다(수치는 버림 시드라
결과가 아니다). **경험적 모집단 평균은 `(5,5,1)` 보다 훨씬 강한 상수 예측기다.**
따라서 G1 은 V1 때보다 확실히 어려워진다. 3-2 에 적은 대로 그대로 간다.
