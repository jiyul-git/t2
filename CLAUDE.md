...
- **계획 층도 집계 %가 아니라 전환 수를 세야 한다.** `stackoff` 에서 집계표는
  ...
  안 움직인다
- **봇이 치는 블러프성 공격의 53%가 계획이 아니라 규율 실패다** (4,000핸드
  실측: 계획 블러프 242건 vs `giveup`/`showdown` 이탈 277건). 이탈 블러프는
  `plan.py:857` 의 "포기 계획이나 지속벳"에서 92%가 나오고, 거기서 쓰는
  `cbet_freq` 는 **핸드 강도·드로우·레인지를 보지 않는** 구조적 빈도다.
  그래서 계획 블러프가 받는 `eq < 0.42` 게이트를 안 거친다 —
  `outs` 중앙 4 → 0, `eq` 90분위 0.418 → 0.540 (`TRACE_BLUFF.md` ③-2)
