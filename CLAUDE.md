...
- **계획 층도 집계 %가 아니라 전환 수를 세야 한다.** `stackoff` 에서 집계표는
  ...
  안 움직인다
- **봇이 치는 블러프성 공격의 53%가 계획 이탈이지만, 비효율적이라는 증거가
  없다.** 4,000핸드에서 필요 폴드율 f* 와 실측 폴드율을 비교하면
  계획 블러프 39.6% vs f* 28.0%(여유 +11.7%p), **이탈 블러프 49.8% vs 22.8%
  (여유 +27.0%p)**. 이탈이 작게 치기 때문이다(0.51팟 vs 0.63팟).
  **두 집단의 우열은 가르지 않는다** — `f*` 가 한 스트리트 모형이라
  턴 배럴이 계획에 포함된 `bluff_2street` 쪽을 과소평가한다.
  **그래서 `cbet_freq` 가 `giveup` 계획에 적용되는 구조는 수정 후보가 아니다** —
  "이 상황은 원래 지속벳 할 자리인가"를 보는 판단이 실측상 맞다
  (`TRACE_BLUFF.md` ③-3, `tools/deviate_ev.py`)
- `SIZING['pot_control']` 은 flop 0.30 / river 0.30 이다. **팟컨트롤 벳은
  이탈이 아니라 계획대로 치는 것** — 이탈 집단에 넣으면 168건이 잘못 세어진다
...
| `tools/deviate_ev.py` | 이탈 공격의 질. 필요 폴드율 f* vs 실측 폴드율 |
